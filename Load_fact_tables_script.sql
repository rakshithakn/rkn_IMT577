...
INSERT INTO FACT_SALESACTUAL (
  ...
  "DimCustomerID", "DimChannelID", 
  "DimSaleDateID", "DimLocationID", 
  "SourceSalesHeaderID", "SourcesSalesDetailID", 
  "SaleAmount", "SaleQuantity", "SaleUnitPrice", 
  ...
  SELECT 
    ...
    (
      dp."ProductCost" * sd.SALESQUANTITY
    ) AS "SalesExtendedCost", 
    (
      sd.SALESAMOUNT - (
        dp."ProductCost" * sd.SALESQUANTITY
      )
  ...
  FROM 
    ...
    JOIN DIM_PRODUCT dp ON sd.PRODUCTID = dp."SourceProductID" 
    JOIN DIM_CHANNEL dch ON TO_CHAR(sh.CHANNELID) = dch."SourceChannelID" 
    JOIN DIM_DATE dd ON sh."DATE" = dd."DATE" 
    LEFT JOIN DIM_STORE ds ON TO_CHAR(sh.STOREID) = ds."SourceStoreID" 
...
-- LOAD PRODUCT SALES TARGET

INSERT INTO FACT_PRODUCTSALESTARGET(
  "DimProductID", "DimTargetDateID", 
  ...
  SELECT 
    ...
    dd.DATE_PKEY, 
    CAST(
      STDP.SALESQUANTITYTARGET / 365 AS INT
    ) AS "DailyTarget" 
  FROM 
    STAGE_TARGETDATAPRODUCT stdp 
    RIGHT JOIN DIM_DATE dd ON stdp."YEAR" = dd."YEAR" 
    JOIN DIM_PRODUCT dp ON dp."SourceProductID" = stdp.PRODUCTID
) 
...
INSERT INTO FACT_SRCSALESTARGET(
  ...
  FROM 
    STAGE_TARGETDATACHANNELSTORE stdc 
    RIGHT JOIN DIM_DATE dd ON stdc."YEAR" = dd."YEAR" 
    LEFT JOIN DIM_STORE ds ON (
      'Store Number ' || ds."StoreNumber"
    ) = stdc.TARGETNAME 
    LEFT JOIN DIM_RESELLER dr ON stdc.TARGETNAME = (
      CASE WHEN dr."ResellerName" = 'Mississipi Distributors' THEN 'Mississippi Distributors' ELSE dr."ResellerName" END
    ) 
    JOIN DIM_CHANNEL dc ON REPLACE(dc."ChannelName", '-', '') = stdc.CHANNELNAME
) 